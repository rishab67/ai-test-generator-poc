...
import os
import json
import time  # NEW: We need this for the 5-second wait
...
def ai_malicious_payload():
    ...
    Include extreme boundary values (like massive integers), incredibly long strings (10,000+ characters), 
    ...
    try:
        ...
        malicious_json = json.loads(clean_text.strip())
        print(f"[🛡️ AppSec] Payload Engineered Successfully!")
        return malicious_json
    except json.JSONDecodeError:
        pytest.fail(f"❌ JSON format failed. Raw response: {response.text}")
